...
# hello mam ,THIS IS WORKING TAKING LANDMARK BUT GIVING THE ERROR OF CV22 ATTRIBUTE ..........
    ...
    if results.multi_hand_landmarks:
        for hand_landmark in results.multi_hand_landmarks:
            ...
            if all(finger_fold_status):
                ...
                image = cv2.cvtColur(np.array(image),cv2.COLOR_RGB2BGR)
                cv2.imwrite("in_memory_to_disk.png",image)

                pyautogui.screenshot("straight_to_disk.png")
                image = cv2.imshow("Screeshot",imutils.resize(image, width = 600))
